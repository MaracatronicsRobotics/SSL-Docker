...
#include <3rdparty/protobuf/packet.pb.h>
#include <3rdparty/protobuf/vssref_common.pb.h>
#include <WRSimActuator/utils/wrtimer.hh>
#include <WRSimActuator/utils/color.hh>

#define WHEELSDIST 0.075
#define WHEELSRADIUS 0.02
#define GEARBOXRATIO 10.0

PacketHandler::PacketHandler(const QString& name, const bool &isSSL) : Actuator(name) {
...
void PacketHandler::sendSSLCommand(uint8 teamNum, uint8 playerNum){
...
void PacketHandler::sendVSSCommand(uint8 teamNum, uint8 playerNum){
    // Sets team and player number to the packet
    fira_message::sim_to_ref::Packet packet;
    fira_message::sim_to_ref::Command *command = packet.mutable_cmd()->add_robot_commands();

    command->set_yellowteam(teamNum == Colors::YELLOW);
    command->set_id(playerNum);

    double playerVy = _vy[teamNum][playerNum];
    double playerVtheta = _vtheta[teamNum][playerNum] * WHEELSDIST / 2.0;

    // Funcional
    double leftWheel = (playerVy - playerVtheta) / WHEELSRADIUS;
    double rightWheel = (playerVy + playerVtheta) / WHEELSRADIUS;

//    double leftWheel = -1000.0f;
//    double rightWheel = -10000.0f;
