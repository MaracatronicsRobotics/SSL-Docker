...
class PacketHandler : public QThread, public Actuator {
	Q_OBJECT
public:
    PacketHandler(const QString &name, const bool &isSSL);
...
private:
    ...
    void sendSSLCommand(uint8 teamNum, uint8 playerNum);
    void sendVSSCommand(uint8 teamNum, uint8 playerNum);

    const double rpm_to_rad_per_s = GEARSystem::Angle::twoPi / 60;
};
